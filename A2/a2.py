...
def model_train(training_images, training_labels, test_images, test_labels):
    #KNN model was used for this task
...
def SVM_main(training_images, training_labels, test_images, test_labels):
    ...
    # This SVM model does not contain all plotting related variables as it was outperformed by KNN
    model_name = "SVM"
...
if features_extracted == True:
    ...
    # Running models
    KNN_main(tr_X.reshape((np.shape(tr_X)[0], 68*2)), list(zip(*tr_Y))[0], te_X.reshape((np.shape(te_X)[0], 68*2)), list(zip(*te_Y))[0])
