...
if features_extracted == False:
    ...
    pred = img_SVM(
        tr_X.reshape((np.shape(tr_X)[0], 3 * 2)),
        list(zip(*tr_Y))[0],
        te_X.reshape((np.shape(te_X)[0], 3 * 2)),
        list(zip(*te_Y))[0],
    )
    ...
    model_test(tr_X.reshape((np.shape(tr_X)[0], 3*2)), list(zip(*tr_Y))[0], te_X.reshape((np.shape(te_X)[0], 3*2)), list(zip(*te_Y))[0])
    # pred=img_SVM(tr_X.reshape((np.shape(tr_X)[0], 3*2)), list(zip(*tr_Y))[0], te_X.reshape((np.shape(te_X)[0], 3*2)), list(zip(*te_Y))[0])
    # pred=img_KNN(tr_X.reshape((np.shape(tr_X)[0], 3*2)), list(zip(*tr_Y))[0], te_X.reshape((np.shape(te_X)[0], 3*2)), list(zip(*te_Y))[0])
    # pred = img_FOREST(
    #     tr_X.reshape((np.shape(tr_X)[0], 3 * 2)),
    #     list(zip(*tr_Y))[0],
    #     te_X.reshape((np.shape(te_X)[0], 3 * 2)),
